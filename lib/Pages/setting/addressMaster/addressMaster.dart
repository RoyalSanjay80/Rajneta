...
class Addressmaster extends StatefulWidget {
  final String selectedLanguage;
  Addressmaster({Key? key, required this.selectedLanguage}) : super(key: key);
  @override
  _AddressmasterState createState() => _AddressmasterState();
}


class _AddressmasterState extends State<Addressmaster> {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        trailing: PopupMenuButton<String>(
          ...
          itemBuilder: (context) => [
            _buildMenuItem('Address Master', 'address_master'),
          ],
          icon: Icon(Icons.more_vert, color: Colors.grey[700]),

        ),
        onTap: (){
          if (user['master_permission']?['address_master'] == 1) {
            // Get.to(() => Addaddress());
            Get.to(() => Addaddress(userid:user['id'],selectedLanguage:widget.selectedLanguage));
          } else {
            Get.snackbar(
              "Access Denied",
              "This user does not have address_master permission",
              backgroundColor: Colors.red,
              colorText: Colors.white,
              snackPosition: SnackPosition.BOTTOM,
            );
