...
import 'byDemands.dart';

class AllDemandsList extends StatefulWidget {
  ...
  @override
  _AllDemandsListState createState() => _AllDemandsListState();
}

class _AllDemandsListState extends State<AllDemandsList> {
  bool isLoading = true;
  ...
  Future<void> fetchData() async {
    try {
      ...
      final responce = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/all-demands-list?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}'),
        headers: {'Authorization': 'Bearer $token'},
      );
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
        if (data['demands'] is List) {
          setState(() {
            voterData =
                List<Map<String, String>>.from(data['demands'].map((item) => {
                      'name': item['demand']?.toString() ?? 'null',
  ...
  Future<void> searchData(String query) async {
    ...
    try {
      ...
      final responce = await http.get(
          Uri.parse(
              '${ApiConstants.baseUrl}api/search-demands?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&demands=$query'),
          headers: {'Authorization': 'Bearer $token'});
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
          ...
          setState(() {
            voterData =
                List<Map<String, String>>.from(data['voters'].map((item) => {
                      'name': item['demands']?.toString() ?? 'null',
