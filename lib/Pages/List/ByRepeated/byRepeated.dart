...
class Byrepeated extends StatefulWidget {
  ...
  @override
  State<Byrepeated> createState() => _ByrepeatedState();
}

class _ByrepeatedState extends State<Byrepeated> {
  bool isLoading = true;
  ...
  Future<void> fetchData() async {
    try {
      ...
      if (token == null) throw Exception('auth_token');
      setState(() {
        isLoading = true;
      });
      final responce = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/repeated-nonrepeated-voters-details?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&status=${widget.voterName}'),
  ...
  Future<void> searchvotres(String query) async {
    if (query.isEmpty) {
      fetchData();
      return;
    }
