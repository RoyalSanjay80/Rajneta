...
import 'byPosition.dart';

class AllPosition extends StatefulWidget {
  ...
  @override
  _AllPositionState createState() => _AllPositionState();
}

class _AllPositionState extends State<AllPosition> {
  ...
  Future<void> fetchData() async {
    try {
      ...
      if (token == null) throw Exception('token is not fount');
      setState(() {
        isLoading = true;
      });
      final responce = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/all-position-list?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}'),
      ...
      if (responce.statusCode == 200) {
        ...
        if (data['position'] is List) {
          setState(() {
            voterData =
                List<Map<String, String>>.from(data['position'].map((item) => {
                      'name': item['position']?.toString() ?? 'null',
  ...
  Future<void>searchvoters(String query)async{
    ...
    try{
      ...
      final responce=await http.get(Uri.parse('${ApiConstants.baseUrl}api/search-position?lang=${widget.selectedLanguage.substring(0,2).toLowerCase()}&position=$query'),
        headers: {'Authorization':'Bearer $token'},
      );
      if(responce.statusCode==200){
        final data=json.decode(responce.body);
        if(data['voters'] is List){
          setState(() {
            voterData=List<Map<String,String>>.from(
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 4),
                    Text(
                      voter['details'] ?? 'No Details Number Available',
        ...
        FloatingActionButton.extended(
          ...
          onPressed: (){
            controller.generateExcel(context,
                voterData.map((item)=>item['name']?? 'null').toList(),
                voterData.map((item)=>item['details']?? 'null').toList(),
                widget.itemName.tr,
                'Total'.tr);
          },
