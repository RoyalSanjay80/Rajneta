...
import 'byColorCode.dart';

class AllColorCode extends StatefulWidget {
  ...
  @override
  _AllColorCodeState createState() => _AllColorCodeState();
}


class _AllColorCodeState extends State<AllColorCode> {
  ...
  Future<void> fetchdata() async {
    try {
      ...
      final responce = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/colour-code-list?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}'),
        headers: {'Authorization': 'Bearer $token'},
      );
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
        if (data['colourcodes'] is List) {
          setState(() {

            votersData = List<Map<String, String>>.from(
                data['colourcodes'].map((item) => {
                      'name': item['color_name']?.toString() ?? 'null',
                      'details': item['voter_count']?.toString() ?? 'null'
                    }));
            isLoading = false;
          });
        } else {
          throw Exception('Unexpected Api responce Strucuter');
        }
      } else {
  ...
  Future<void> serachvoters(String query) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/search-colour-code?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&colour_code=$query'),
        headers: {'Authorization': 'Bearer $token'},
      );
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          votersData =
              List<Map<String, String>>.from(data['colourCodes'].map((item) => {
                    'name': item['color_name']?.toString() ?? 'null',
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) => Bycolorcode(
              voterName: voter['name']!.tr,
              voterDetails: voter['details']!.tr,
