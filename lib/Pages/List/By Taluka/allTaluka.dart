...
import 'byTaluka.dart';

class AllTaluka extends StatefulWidget {
  ...
  @override
  _AllTalukaState createState() => _AllTalukaState();
}

class _AllTalukaState extends State<AllTaluka> {
  bool isloading=true;
  ...
  Future<void>fetchData()async{
    try{
      ...
      final responce=await http.get(Uri.parse('${ApiConstants.baseUrl}api/all-taluka-list?lang=${widget.selectedLanguage.substring(0,2).toLowerCase()}'),
          headers: {'Authorization':'Bearer $token'}
      );
      if(responce.statusCode==200){
        final data=json.decode(responce.body);
        if(data['talukanos']is List){
          setState(() {
            votersData=List<Map<String,String>>.from((data['talukanos']as List).map((item)=>{
              'name':item['taluka']?.toString() ?? 'null',
  ...
  Future<void>searchData(String query)async{
    ...
    try{
      ...
      final responce=await http.get(Uri.parse('${ApiConstants.baseUrl}api/search-taluka?lang=${widget.selectedLanguage.substring(0,2).toLowerCase()}&taluka=$query'),
          headers: {'Authorization':'Bearer $token'}
      );
      if(responce.statusCode==200){
        final data=json.decode(responce.body);
        if(data['talukas']is List){
          setState(() {
            votersData=List<Map<String,String>>.from((data['talukas']as List).map((item)=>{
              'name':item['taluka']?.toString() ?? 'null',
              'details':item['total_voters']?.toString() ?? 'null',
            }));
            isloading=false;
