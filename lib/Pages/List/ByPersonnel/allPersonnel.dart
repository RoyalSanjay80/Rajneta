...
class AllPersonnel extends StatefulWidget {
  ...
  AllPersonnel({Key? key, required this.itemName, required this.selectedLanguage})
      : super(key: key);

  @override
  _AllPersonnelState createState() => _AllPersonnelState();
}

class _AllPersonnelState extends State<AllPersonnel> {
    bool isLoading=true;
    List<Map<String,String>>votersData=[];
    final Subcontroller controoler =Get.put(Subcontroller());
  ...
  void initState() {
    ...
    Future<void> fetchData() async {
      try {
        ...
        if (token == null) throw Exception('auth_token not found');
        ...
        final response = await http.get(
          Uri.parse('${ApiConstants.baseUrl}api/personnel-list'),
            ...
            setState(() {
              votersData = List<Map<String, String>>.from(
                data['voters'].map((item) => {
                  'status': item['status']?.toString() ?? 'null',
                  'voter_count': item['voter_count']?.toString() ?? 'null'
                }),
              );
              isLoading = false;
            });
          ...
          child: Row(
            ...
            children: [
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 4),
                    Text(
                      voter['voter_count'] ?? 'No phone number available',
              ...
              IconButton(
                  ...
                  onPressed: () {
                    controoler.showOptionsDialog(context,
                        widget.itemName,
    ...
    return Column(
      ...
      children: [
        FloatingActionButton.extended(
          heroTag: 'exportFAB',
          onPressed:(){
            controoler.generateExcel(context,
        ...
        FloatingActionButton.extended(
          heroTag: 'reportFAB',
          onPressed: (){
            controoler.generatePDF(context,
                votersData.map((item)=>item['status']?? 'null').toList(),
                votersData.map((item)=>item['voter_count']?? 'null').toList(),
               widget.itemName,
                'Total'.tr);
