...
class Allvillagelist extends StatefulWidget {
  final String villagename ;
  final String selectedLanguage ;
  final String totlevillage ;
  const Allvillagelist({super.key, required this. villagename, required this.selectedLanguage, required this. totlevillage});

  @override
  State<Allvillagelist> createState() => _AllvillagelistState();
}

class _AllvillagelistState extends State<Allvillagelist> {
  bool isLoading = true;
  ...
  Future<void> fetchVotersData() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/voter-details-by-village?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&village=${widget.villagename}'),
          ...
          setState(() {
            ...
            votersData = List<Map<String, String>>.from(data['voters'].map((dynamic item) {
              ...
              return {
                ...
                'details': voterDetails['id']?.toString() ?? 'No details available',
                'voterId': voterDetails['voter_id']?.toString() ?? 'No voter ID available',
             ...
             isLoading
               ...
               child: Column(
                 children: [
                   ...
                   Text("please wait loading",style: TextStyle(fontSize: 20,color: Colors.white),)
                 ],
               ),
             )
                 : Expanded(
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              '${widget.villagename.tr}  ${widget.totlevillage.tr}',
              style: TextStyle(
                  fontSize: 24,
