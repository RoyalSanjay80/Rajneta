...
class Byextracheck2 extends StatefulWidget {
  ...
  @override
  State<Byextracheck2> createState() => _Byextracheck2State();
}

class _Byextracheck2State extends State<Byextracheck2> {
  bool isLoading = true;
  ...
  Future<void> fetchData() async {
    try {
      ...
      final responce = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/extra-check-two-voters-details?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&status=${widget.voterName}'),
        ...
        if (data['voters'] is List) {
          setState(() {
            votersData = List<Map<String, String>>.from(
              ...
              return {
                'name': [
                  votersDeatils['first_name'] ?? 'Null',
                  votersDeatils['middle_name'] ?? 'Null',
                  votersDeatils['surname'] ?? 'Null',
                ].where((namepart) => namepart.isNotEmpty).join(' '),
                'details': votersDeatils['id']?.toString() ?? 'null',
                'voterId': votersDeatils['voter_id']?.toString() ?? 'null',
  ...
  Future<void> serachData(String query) async {
    ...
    try{
      SharedPreferences preferences = await SharedPreferences.getInstance();
      String? token = preferences.getString('auth_token');
      if (token == null) throw Exception('auth toke is not fount');
      setState(() {
        isLoading = true;
      });
      final responce = await http.get(
        Uri.parse('${ApiConstants.baseUrl}api/search-by-first-middle-surname?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&name=$query'),
        headers: {'Authorization': 'Bearer $token'},
      );
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
