...
class ByCast extends StatefulWidget {
  ...
  ByCast({Key? key, required this.itemName, required this.selectedLanguage})
      : super(key: key);

  @override
  _ByCastState createState() => _ByCastState();
}
...
class _ByCastState extends State<ByCast> {
  ...
  Future<void> fetchData() async {
    try {
      ...
      final responce = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/all-cast-list?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}'),
        headers: {'Authorization': 'Bearer $token'},
      );
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
        if (data['caste'] is List) {
          setState(() {
            votersData =
                List<Map<String, String>>.from(data['caste'].map((item) => {
                      'name': item['caste']?.toString() ?? ' null',
  ...
  Future<void>searchvoters(String query)async{
    ...
    try{
      ...
      String? token=preferences.getString('auth_token');
      if(token==null) throw Exception('auth token is not found');
       final responce=await http.get(Uri.parse('${ApiConstants.baseUrl}api/search-caste?lang=${widget.selectedLanguage.substring(0,2).toLowerCase()}&caste=$query'),
       headers: {'Authorization': 'Bearer $token'}
       );
       if(responce.statusCode==200){
         final data=json.decode(responce.body);
         if(data['voters'] is List){
           setState(() {
             votersData=List<Map<String,String>>.from(data['voters'].map((item)=>{
               'name':item['castes']?.toString() ?? 'null',
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      ...
                      voter['name'] ?? 'No voters name avilable',
                      style:
