...
class Byextrainfo1 extends StatefulWidget {
  ...
  @override
  State<Byextrainfo1> createState() => _Byextrainfo1State();
}

class _Byextrainfo1State extends State<Byextrainfo1> {
  bool isLoading = true;
  ...
  Future<void> fetchdata() async {
    try {
      ...
      if (token == null) throw Exception('auth token is nt found');
      setState(() {
        isLoading = true;
      });
      final responce = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/voter-details-by-extra-information-one?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&extra_info_1=${widget.voterName}'),
        ...
        if (data['voters'] is List) {
          setState(() {
            votersData = List<Map<String, String>>.from(
              ...
              final voterDeatils = item['voter_details'] ?? {};
              final addressDetails = item['address_details'] ?? {};
              return {
                'name': [
                  voterDeatils['first_name'] ?? 'null',
                  voterDeatils['middle_name'] ?? 'null',
                  voterDeatils['surname'] ?? 'null',
                ].where((namepart) => namepart.isNotEmpty).join(' '),
                'details': voterDeatils['id']?.toString() ?? 'null',
                'voterId': voterDeatils['voter_id']?.toString() ?? 'null',
