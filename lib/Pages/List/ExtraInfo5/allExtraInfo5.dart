...
import 'byExtraInfo5.dart';

class AllExtraInfo5 extends StatefulWidget {
  ...
  @override
  _AllExtraInfo5State createState() => _AllExtraInfo5State();
}

class _AllExtraInfo5State extends State<AllExtraInfo5> {
  bool isloading = true;
  List<Map<String, String>> votersData = [];
  String query = '';
  Subcontroller controlle = Get.put(Subcontroller());
  ...
  Future<void> fetchData() async {
    try {
      ...
      final responce = await http.get(
          Uri.parse(
              '${ApiConstants.baseUrl}api/extra-information-five-list?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}'),
          headers: {'Authorization': 'Bearer $token'});
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
        if (data['extra_information_five_details'] is List) {

          setState(() {
            votersData = List<Map<String, String>>.from(
                (data['extra_information_five_details'] as List).map((item) {
              return {
                'name': item['extra_information_five']?.toString() ?? 'null',
                'details':
                    item['extra_information_five_count']?.toString() ?? 'null',
              };
            }));
            isloading = false;
          });
        } else {
          throw Exception('Unexpeted Api Responce');
        }
      } else {
  ...
  Future<void> searchData(String query) async {
    ...
    try {
      ...
      final responce = await http.get(
          Uri.parse(
              '${ApiConstants.baseUrl}api/search-extra-information-five?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&extra_info_5=$query'),
          headers: {'Authorization': 'Bearer $token'});
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
        if (data['extra_info_five_voters'] is List) {
          setState(() {
            votersData = List<Map<String, String>>.from(
                (data['extra_info_five_voters'] as List).map((item) {
              return {
                'name': item['extra_info_five']?.toString() ?? 'null',
    ...
    return Column(
      ...
      children: [
        FloatingActionButton.extended(
          heroTag: 'exportFAB',
          onPressed: (){
            controlle.generateExcel(context,
