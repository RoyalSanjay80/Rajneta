...
import 'byGender.dart';

class AllGender extends StatefulWidget {
  final String itemName;
  final String selectedLanguage;

  AllGender({Key? key, required this.itemName, required this.selectedLanguage})
      : super(key: key);

  @override
  _AllGenderState createState() => _AllGenderState();
}

class _AllGenderState extends State<AllGender> {
  ...
  Future<void> fetchData() async {
    try {
      ...
      final responce = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/gender-list?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}'),
        headers: {'Authorization': 'Bearer $token'},
      );
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
        if (data['gender'] is List) {
          setState(() {
            votersData =
                List<Map<String, String>>.from(data['gender'].map((item) => {
                      'name': item['gender']?.toString() ?? 'null',
  ...
  Future<void> seracData(String query) async {
    if (query.isEmpty) {
      fetchData();
      return;
    }
    try {
      ...
      final responce = await http.get(
        Uri.parse('${ApiConstants.baseUrl}api/search-gender?lang=${widget.selectedLanguage.substring(0,2).toLowerCase()}&gender=$query'),
        headers: {'Authorization': 'Bearer $token'},
      );
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
        if (data['genders'] is List) {
          ...
          throw Exception('faid to search data');
        }
      } else {
        throw Exception('faid to search data');
      }
