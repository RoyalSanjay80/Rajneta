...
import 'byNewAddress.dart';

class AllNewAdderss extends StatefulWidget {
  ...
  @override
  _AllNewAdderssState createState() => _AllNewAdderssState();
}

class _AllNewAdderssState extends State<AllNewAdderss> {
  ...
  Future<void> fetchData() async {
    try {
      ...
      final responce = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/all-new-address-list?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}'),
        headers: {'Authorization': 'Bearer $token'},
      );
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
        if (data['new_address'] is List) {
          setState(() {
            votersData = List<Map<String, String>>.from(
                data['new_address'].map((item) => {
                      'name': item['new_address_mr']?.toString() ?? 'null',
                      'details': item['voter_count']?.toString() ?? 'null'
                    }));
            isLoding = false;
          });
        } else {
          throw Exception('unexpected Api Responce Stutcture');
        }
      } else {
  ...
  Future<void> searchnewaddress(String query) async {
    if (query.isEmpty) {
      return fetchData();
    }
    try {
      ...
      final responce = await http.get(
          Uri.parse(
              '${ApiConstants.baseUrl}api/search-new-address?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&new_address=$query'),
          headers: {'Authorization': 'Bearer $token'});
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
        if (data['voters'] is List) {
          setState(() {
            votersData = List<Map<String, String>>.from(
              ...
              return {
                'name': item['new_address']?.toString() ?? 'null',
  ...
  Widget _buildVotersList() {
    return Expanded(
      child: votersData.isEmpty
          ? Center(
              child: Text("Data is Not Available",style: TextStyle(
                fontSize: 20,
                color: Colors.white,
                fontWeight: FontWeight.bold
              ),),
        ...
        FloatingActionButton.extended(
          ...
          onPressed: (){
            controller.generateExcel(context,
                ...
                votersData.map((item)=>item['voter_count']?? 'null').toList(),
                'By New Address',
                'Total');
          },
