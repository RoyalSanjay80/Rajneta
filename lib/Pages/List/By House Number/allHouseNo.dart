...
class AllHouseNo extends StatefulWidget {
  final String itemName;
  final String selectedLanguage;

  AllHouseNo({Key? key, required this.itemName, required this.selectedLanguage})
      : super(key: key);

  @override
  _AllHouseNoState createState() => _AllHouseNoState();
}

class _AllHouseNoState extends State<AllHouseNo> {
  ...
  Future<void> fetchData() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/all-houseno-list?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}'),
        headers: {'Authorization': 'Bearer $token'},
      );

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        if (data['housenos'] is List) {
          setState(() {
            votersData =
                List<Map<String, String>>.from(data['housenos'].map((item) => {
                      'name': item['house_no']?.toString() ?? 'N/A',
                      'details': item['total_voters']?.toString() ?? 'N/A',
                    }));
            isLoading = false;
          });
        } else {
  ...
  Future<void> searchData(String query) async {
    ...
    try {
      ...
      final responce = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/search-houseno?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&house_no=$query'),
        headers: {'Authorization': 'Bearer $token'},
      );
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
        if (data['house_nos'] is List) {
          setState(() {
            votersData =
                List<Map<String, String>>.from(data['house_nos'].map((item) => {
                      'name': item['house_no']?.toString() ?? 'null',
                      'details': item['total_voters']?.toString() ?? 'null',
                    }));
            isLoading = false;
          });
        } else {
          throw Exception('Unexpeted Api Responce Sturcture');
        }
      } else {
          ...
          child: Row(
            ...
            children: [
              ...
              IconButton(
                  ...
                  onPressed: () {
                    controller.showOptionsDialog(context, widget.itemName, widget.selectedLanguage);
                  }),
            ],
          ),
        ),
        ...
        FloatingActionButton.extended(
          ...
          onPressed: (){
            controller.generatePDF(context,
                votersData.map((item)=>item['name'] ??' null').toList(),
                votersData.map((item)=>item['detail'] ?? 'null').toList(),
                widget.itemName.tr,
                'Total'.tr
            );
          },
