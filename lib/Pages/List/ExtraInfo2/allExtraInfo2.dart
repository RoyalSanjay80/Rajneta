...
import 'byExtraInfo2.dart';

class AllExtraInfo2 extends StatefulWidget {
  ...
  @override
  _AllExtraInfo2State createState() => _AllExtraInfo2State();
}

class _AllExtraInfo2State extends State<AllExtraInfo2> {
  ...
  Future<void> fetchData() async {
    try {
      ...
      final responce = await http.get(
        Uri.parse('${ApiConstants.baseUrl}api/extra-information-two-list?lang=${widget.selectedLanguage.substring(0,2).toLowerCase()}'),
        headers: {'Authorization': 'Bearer $token'},
      );
      if (responce.statusCode == 200) {
        final data = json.decode(responce.body);
        if (data['extra_information_two_details'] is List) {
          setState(() {
            votersData = List<Map<String, String>>.from(
                (data['extra_information_two_details'].map((item) => {
                  'name':item['extra_information_two']?.toString() ?? 'null',
                  'details':item['extra_information_two_count']?.toString() ?? 'null',
                })));
            isLoading=false;
          });
        }else{
  ...
  Future<void>searchData(String query)async{
    ...
    try{
      ...
      setState(() {
       ...
       final  responce=await http.get(Uri.parse('${ApiConstants.baseUrl}api/search-extra-information-two?lang=${widget.selectedLanguage.substring(0,2).toLowerCase()}&extra_info_2=$query'),
         headers: {'Authorization':'Bearer $token'},
       );
       if(responce.statusCode==200){
         final data=json.decode(responce.body);
         if(data['extra_info_two_voters'] is List){
           setState(() {
             votersData=List<Map<String,String>>.from((data['extra_info_two_voters'] as List).map((item){
               return{
                 'name':item['extra_info_two']?.toString() ?? 'null',
