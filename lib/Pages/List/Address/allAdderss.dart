...
import 'byAdderss.dart';

class AllAdderss extends StatefulWidget {
  final String itemName;
  final String selectedLanguage;

  AllAdderss({Key? key, required this.itemName, required this.selectedLanguage})
      : super(key: key);

  @override
  _AllAdderssState createState() => _AllAdderssState();
}

class _AllAdderssState extends State<AllAdderss> {
  ...
  Future<void> fetchData() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/address-list?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}'),
          ...
          setState(() {
            votersData = List<Map<String, String>>.from(
              data['addresses'].map((item) => {
                    'name': item['address']?.toString() ?? 'Unknown Address',
  ...
  Future<void> serachData(String query) async {
    ...
    try {
      ...
      final responce = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/search-address?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&address=$query'),
        headers: {'Authorization': 'Bearer $token'},
      );
      if (responce.statusCode == 200) {
        ...
        setState(() {
          votersData =
              List<Map<String, String>>.from(data['addresses'].map((item) => {
                    'name': item['address']?.toString() ?? 'null',
        ...
        throw Exception('faild to search voters');
      }
    } catch (e) {
      setState(() {
        isLoding = false;
