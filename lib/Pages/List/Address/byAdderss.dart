...
class Byadderss extends StatefulWidget {
  ...
  @override
  State<Byadderss> createState() => _ByadderssState();
}

class _ByadderssState extends State<Byadderss> {
  bool isLoding = true;
  ...
  Future<void> fetchData() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/voter-details-by-address?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&address=${Uri.encodeComponent(widget.voterName)}'),
      ...
      if (response.statusCode == 200) {
          ...
          setState(() {
            ...
            votersData = List<Map<String, String>>.from(
              data['voters'].map((dynamic item) {
                ...
                final addressDetails = item['address_details'] ?? {}; // Check for address key
                ...
                return {
                  ...
                  'details': voterDetails['id']?.toString() ?? 'N/A',
                  'voterId': voterDetails['voter_id']?.toString() ?? 'N/A',
        ...
        throw Exception('Failed to load voter data. Status Code: ${response.statusCode}');
      }
    } catch (e) {
      setState(() {
        isLoding = false;
