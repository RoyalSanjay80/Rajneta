...
class AllVotingCenter extends StatefulWidget {
  final String itemName;
  final String selectedLanguage;



  AllVotingCenter({Key? key, required this.itemName, required this.selectedLanguage})
      : super(key: key);

  @override
  _AllVotingCenterState createState() => _AllVotingCenterState();
}

class _AllVotingCenterState extends State<AllVotingCenter> {
  final Subcontroller contrller=Get.put(Subcontroller());
  final Printcontroller1 covariant1=Get.put(Printcontroller1());
  ...
  Future<void>fetchdata()async{
    ...
    try{
      ...
      final responce=await http.get(Uri.parse('${ApiConstants.baseUrl}api/voting-centre-list?lang=${widget.selectedLanguage.substring(0,2).toLowerCase()}',
      ),
        headers: {'Authorization':'Bearer $token'},
      );
      if(responce.statusCode==200){
        final data=json.decode(responce.body);
        if(data['voting_centers'] is List){
          setState(() {
            votersdata=List<Map<String,String>>.from(
              data['voting_centers'].map((item)=>{
                'name':item['voting_centre']?.toString() ?? 'null',
    ...
    try{
      ...
      setState(() {
       ...
       final response= await http.get(
         Uri.parse('${ApiConstants.baseUrl}api/search-voting-centre?lang=${widget.selectedLanguage.substring(0,2).toLowerCase()}&voting_centre=$query'),
         headers: {'Authorization': 'Bearer $token'},
       );
