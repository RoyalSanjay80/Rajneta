...
class Byvotingcenter extends StatefulWidget {
  final String voterName;
  final String voterDetails;
  final String selectedLanguage;
  const Byvotingcenter(
      {super.key,
  ...
  @override
  State<Byvotingcenter> createState() => _ByvotingcenterState();
}

class _ByvotingcenterState extends State<Byvotingcenter> {
  bool isLoding = true;
  ...
  Future<void> fetchdata() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
            '${ApiConstants.baseUrl}api/voter-details-by-voting-centre?lang=${widget.selectedLanguage.substring(0, 2).toLowerCase()}&voting_centre=${widget.voterName}'),
      ...
      if (response.statusCode == 200) {
            ...
            votersdata = List<Map<String, String>>.from(
              data['voters'].map((dynamic item) {
                ...
                return {
                  ...
                  'dob': voterDetails['dob']?.toString() ?? 'Unknown DOB',
                };
              }).toList(),
            );
          });
        ...
        throw Exception('Failed to load voter data. Status: ${response.statusCode}');
      }
    } catch (e) {
      setState(() {
        isLoding = false;
