...
class AllVoted extends StatefulWidget {
  final String itemName;
  final String selectedLanguage;



  AllVoted({Key? key, required this.itemName, required this.selectedLanguage})
      : super(key: key);

  @override
  _AllVotedState createState() => _AllVotedState();
}

class _AllVotedState extends State<AllVoted> {
  bool isloading=true;
  ...
  Future<void>fetchData()async{
   try{
     ...
     final responce=await http.get(Uri.parse('${ApiConstants.baseUrl}api/voted-list'),
         ...
         setState(() {
           votersData=List<Map<String,String>>.from(data['voters'].map((item)=>{
             'name':item['status']?.toString() ?? 'null',
              'details':item['voter_count']?.toString() ?? 'No Detils Available'
           }));
           isloading=false;
         });
       }else{
